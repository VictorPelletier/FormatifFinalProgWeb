...
export class AppComponent {
  ...
  connectToHub() {
    ...
    this.hubConnection.on('IncreasePlayerChoices', (choiceIndex:number) => {
      this.zone.run(() => {
    ...
    this.hubConnection.on('PlayerRightAnswer',() =>{
      this.zone.run(() => {
        alert("Bonne réponse!");
        this.nbRightAnswers++;
      });
    ...
    this.hubConnection.on('PlayerWrongAnswer', (rightAnswerIndex: number) => {
  this.zone.run(() => {
    const correctAnswer = this.currentQuestion!.answers[rightAnswerIndex];
